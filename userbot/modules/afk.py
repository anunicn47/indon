...
async def mention_afk(mention):
    ...
    if ISAFK and mention.message.mentioned:
        ...
        if day == 1:
            ...
            afk_str = f"`{int(hours)} hours {int(minutes)} minutes` ago"
        elif minutes > 0:
            afk_str = f"`{int(minutes)}minutes {int(seconds)}seconds` ago"
        else:
            afk_str = f"`{int(seconds)} seconds` ago"
...
async def type_afk_is_not_true(notafk):
    ...
    if ISAFK:
        ISAFK = False
        msg = await notafk.edit("**Assamualaikum aing kambek xixi**")
